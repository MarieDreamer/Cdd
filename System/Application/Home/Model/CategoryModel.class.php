...
class CategoryModel extends CommonMAbstract {
    ...
    public function dele(){
        ...
        public function raedit(){
            ...
            //正整数正则
            $shuziyz='/^\d+$/';
            if($name && $category_img && $id){
                if(preg_match($shuziyz, $pid) && preg_match($shuziyz, $id)){
                    $data=array();
                    $data['pid']=$pid;
                    $data['name']=$name;
                    $data['category_img']=json_encode($category_img);
    ...
    public function adds2(){
        ...
        $shuziyz='/^\d+$/';
        if($name && $category_img){
            if(preg_match($shuziyz, $pid)){
                $data=array();
                $data['pid']=$pid;
                $data['name']=$name;
                $data['category_img']=$category_img;
                $data['introduce']=$introduce;
                $data['status_delete']=1;
                $data['time']=time();
                if(!$this->add($data)){
